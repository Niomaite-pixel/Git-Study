...
namespace ЛР1
{
    internal class Program
    {
        ...
        public static void Log(int attempts)
        {
            string logText = $"Победа достигнута за {attempts} попыток в {DateTime.Now}\n";
        ...
        public static void UpdateBestResult(int attempts)
        {
            ...
            if (File.Exists(logFile))
            {
                ...
                if (int.TryParse(bestResultLine.Split(' ')[3], out int bestAttempts))
                {
                    ...
                    {
                        File.WriteAllLines(logFile, new[] { $"Лучший результат: {attempts} попыток" });
        ...
        public static void DisplayBestResult()
        {
            if (File.Exists(logFile))
            {
                ...
                Console.WriteLine("Лучший результат: " + bestResultLine);
            }
            else
            {
